...
import { LabelList, LineChart, Line, XAxis, Tooltip, ResponsiveContainer} from "recharts";
const LinePlot = (props) => {
    const getGraph = () => {
        ...
        return <ResponsiveContainer width="100%" height={props.options.height}>
            <LineChart data={props.data} height={props.options.height} margin={{ top: props.options.margin_top, right: props.options.margin_right, left: props.options.margin_left, bottom: props.options.margin_bottom}}> 
                <XAxis dataKey={props.header.id[0]} />
                {grid}
                {legend}
                {yAxis}
                <Tooltip formatter={(value) => new Intl.NumberFormat('pt').format(value)}/>
                <Line isAnimationActive={true} dataKey={props.header.value[0]} fill={props.options.colors[0]} stroke={props.options.colors[0]} type={interpolation} strokeWidth={props.options.lineStroke} dot={props.options.dots} fillOpacity={props.options.opacity/100}>
